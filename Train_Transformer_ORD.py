...
for mbr,brn in mbrnlist:
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        end_time = timer()
        val_loss, acc, prec, reca, f1sc, confusion = evaluate(transformer, Xtest_data, Ytest_data, loss_fn, device,
                                                              BATCH_SIZE, bptt)
        print((
    ...
    PATH = 'results/best_model_Trans_'+ str(mbr)+'_'+str(brn)+ trainname
    torch.save(best_model.state_dict(), PATH)

    file_name = 'results/result_Trans_' + str(mbr)+'_'+str(brn) + trainname + '.txt'
    ...
    with open("results/Val_loss_Trans_"+ str(mbr)+'_'+str(brn)+ trainname, "wb") as fp:  # Pickling
        pickle.dump(Val_loss, fp)

    with open("results/Train_loss_Trans_"+ str(mbr)+'_'+str(brn)+ trainname, "wb") as fp:  # Pickling
        pickle.dump(Train_loss, fp)

    with open("results/Accuracy_Trans_"+ str(mbr)+'_'+str(brn)+ trainname, "wb") as fp:  # Pickling
        pickle.dump(Accuracy, fp)

    with open("results/F1_Trans_"+ str(mbr)+'_'+str(brn)+ trainname, "wb") as fp:  # Pickling

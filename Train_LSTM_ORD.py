...
for mbr,brn in mbrnlist:
    ...
    lstm = RNNModel(rnn_type='LSTM', ntoken=SRC_VOCAB_SIZE, ninp=EMB_SIZE, nhid=FFN_HID_DIM, nlayers=NUM_LAYERS,
    ...
    PATH='results/best_model_alstm_seq_'+ str(mbr)+'_'+str(brn)+ date_time
    #     if featnorm==True:
    #         torch.save(best_model.state_dict(), PATH+'norm')
    #     else:
    #         torch.save(best_model.state_dict(), PATH)

    file_name='results/result_ALSTM_'+ str(mbr)+'_'+str(brn)+ date_time+'.txt'
    ...
    with open("results/Val_loss_ALSTM_"+ str(mbr)+'_'+str(brn)+ date_time, "wb") as fp:   #Pickling
        pickle.dump(Val_loss, fp)
    with open("results/Train_loss_ALSTM_"+ str(mbr)+'_'+str(brn)+ date_time, "wb") as fp:   #Pickling
        pickle.dump(Train_loss, fp)
    with open("results/Accuracy_ALSTM_"+ str(mbr)+'_'+str(brn)+ date_time, "wb") as fp:   #Pickling
        pickle.dump(Accuracy, fp)
    with open("results/F1_ALSTM_"+ str(mbr)+'_'+str(brn)+ date_time, "wb") as fp:   #Pickling
